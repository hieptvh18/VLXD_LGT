                        ...
                        <div class="col s12 m4 l4">
                            ...
                            <div class="card animate fadeLeft">
                                <div class="card-content">
                                    <h6 class="mb-0 mt-0 display-flex justify-content-between">BDS Đã bán <i
                                            class="material-icons float-right">more_vert</i>
                                    </h6>
                                    <p class="medium-small">Số lượng BDS đã bán</p>
                                    <div class="current-balance-container">
                                        <div id="current-balance-donut-chart" class="current-balance-shadow"></div>
                                    </div>
                                    <h5 class="center-align">{{ $totalAmountItemSold }}</h5>
                                    <p class="medium-small center-align">Tổng doanh thu</p>
                                </div>
                            </div>
                        </div>

                        <div class="col s12 m6 l8">
                            <div class="card subscriber-list-card animate fadeRight" style="min-height: 330px;">
                                <div class="card-content pb-1">
                                    <h4 class="card-title mb-0">Tài khoản đăng nhập hệ thống gần đây <i class="material-icons float-right">more_vert</i>
                                    </h4>
                                </div>
                                <table class="subscription-table responsive-table highlight">
                                    ...
                                    <tbody>
                                        @foreach($listUserLastLogined as $user)
                                            <tr>
                                                <td>{{ $user->name }}</td>
                                                <td>{{ $user->email }}</td>
                                                <td>{{ $user->last_activity ? \Illuminate\Support\Carbon::parse($user->last_activity)->format('d-m-Y H:i:s') : '' }}</td>
                                                <td><span class="badge green lighten-5 red-text text-accent-2" style="text-wrap: nowrap">{{ \App\Helpers\Constant::USER_ROLE_TEXT[$user->role] ?? '' }}</span></td>
                                                <td>{{ $user->ip }}</td>
                                                <td class="center-align">
                                                    <a href="{{ route('admin.user.edit', ['user' => $user->id]) }}"><i
                                                            class="material-icons primary-text">remove_red_eye</i></a>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                    ...
                    <div class="row">
                        <div class="col s12 m6 l4">
                            <div class="card padding-4 animate fadeLeft">
                                <div class="row ">
                                    <div class="col s5 m5">
                                        <h5 class="mb-0">{{ $totalAccount }}</h5>
                                        <p class="no-margin">Tài khoản</p>
                                        <p class="mb-0 pt-8">{{ $totalAccountCustomer }}</p>
                                    </div>
                                ...
                                <div class="row">
                                    <div class="col s5 m5">
                                        ...
                                        <p class="no-margin">Tin tức</p>
                                        <p class="mb-0 pt-8">{{ $totalNewsPublish }}</p>
                                    </div>
                                    <div class="col s7 m7 right-align">
                                        <i
                                            class="material-icons background-round mt-5 mb-5 gradient-45deg-purple-amber gradient-shadow white-text">assessment</i>
                                        <p class="mb-0">Tin đang đăng</p>
                                    </div>
                                </div>
                                <br>
                                <div class="row">
                                    <div class="col s5 m5">
                                        <h5 class="mb-0">{{ $totalItem }}</h5>
                                        <p class="no-margin">BDS Đang có</p>
                                        <p class="mb-0 pt-8"></p>
                                    </div>
                                    <div class="col s7 m7 right-align">
                                        <i
                        ...
                        <div class="col s12 m6 l8">
                            <div class="card subscriber-list-card animate fadeRight">
                                <div class="card-content pb-1">
                                    <h4 class="card-title mb-0">Tin tức mới nhất <i class="material-icons float-right">more_vert</i>
                                    </h4>
                                </div>
                                <table class="subscription-table responsive-table highlight">
                                    <thead>
...
{{--                                        <th>Tóm tắt</th>--}}
                                    ...
                                    <tbody>
                                    @foreach($listNewsNewest as $news)
                                        <tr>
                                            <td title="{{ $news->title }}">{{ formatSubStr($news->title, 50) }}</td>
                                            <td>
                                                <img src="{{ $news->getFirstMediaUrl('featured_image') }}" alt="Featured Image"
                                                     class="circle materialboxed" style="width: 40px; height: 40px;" />
                                            </td>
{{--                                            <td title="{{ $news->short_desc }}">{{ substr($news->short_desc, 0, 100) }}{{ mb_strlen($news->short_desc, 'UTF-8') > mb_strlen(substr($news->short_desc, 0, 100), 'UTF-8') ? '...' : '' }}</td>--}}
                                            <td>
                                                    <span style="text-wrap: nowrap" class="badge green lighten-5 green-text text-accent-4">{{ \App\Helpers\Constant::NEWS_STATUS_TEXT[$news->status] }}
                                                    </span>
                                            </td>
                                            <td>{{ $news->created_at ? \Carbon\Carbon::parse($news->created_at)->format('d-m-Y') : '' }}</td>
                                            <td class="center-align"><a href="{{ route('admin.news.edit', ['news' => $news->id]) }}"><i
                                                        class="material-icons pink-text">remove_red_eye</i></a></td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                </table>
        ...
        $(document).ready(function () {
            ...
            var CurrentBalanceDonutChart = new Chartist.Pie(
                "#current-balance-donut-chart",
                {
                    labels: [1, 2],
                    series: [
                        { meta: "Tổng số BDS", value: {{ (100 - $countItemSoldRate) }} },
                        { meta: "BDS đã bán", value: {{ $countItemSoldRate }} }
                    ]
                },
                    ...
                    plugins: [
                        ...
                        Chartist.plugins.fillDonut({
                            items: [
                                {
                                    content:
                                        '<p class="small">BDS</p><h5 class="mt-0 mb-0">{{ $countItemSold }}</h5>'
                                }
                            ]
                        })
                    ]
